...
def test_product_service_enables_the_fused_temporal_pipeline() -> None:
    ...
    assert "AGENTMEMORY_SPARSE_PATH=/bertoni/data/agent-memory-fts.sqlite3" in environment
...
def test_product_service_locks_the_physical_tenant_database_contract() -> None:
    ...
    assert "bertoni-data:/bertoni" in product["volumes"]
    assert "bertoni-data" in compose["volumes"]
    assert (
        "AGENTMEMORY_TENANT_REGISTRY_PATH=${AGENTMEMORY_TENANT_REGISTRY_PATH:-"
        "/bertoni/data/agent-memory-tenant-registry.sqlite3}"
    ) in environment
