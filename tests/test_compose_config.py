...
def test_product_service_enables_the_fused_temporal_pipeline() -> None:
    ...
    assert "TURINGDB_MEMORY_INDEX=agent_memory_episode_vectors_768" in environment
    assert "TURINGDB_DOCUMENT_INDEX=agent_memory_document_vectors_768" in environment
    assert "TURINGDB_ENTITY_INDEX=agent_memory_entity_vectors_768" in environment
    assert "TURINGDB_FACT_INDEX=agent_memory_fact_vectors_768" in environment
    assert "TURINGDB_COMMUNITY_INDEX=agent_memory_community_vectors_768" in environment
