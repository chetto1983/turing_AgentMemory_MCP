...
| 05-04-02 | 04 | 2 | ARC-07 | T-05-04-01/02/03/04/05 | GREEN proves exact authenticated server lifecycle commands/decoding/failure propagation, reconciles create races, and sources manifest `created_at` from the durable registry winner | unit/integration TDD GREEN | `python -m pytest tests/test_tenant_provisioning.py tests/test_arcadedb_client_transport.py tests/test_arcadedb_schema.py -q && python -m ruff check src/turing_agentmemory_mcp/tenant_provisioning.py src/turing_agentmemory_mcp/arcadedb_client.py src/turing_agentmemory_mcp/arcadedb_schema.py tests/test_tenant_provisioning.py tests/test_arcadedb_client_transport.py && bash scripts/check-file-size.sh` | Task extends the scripted transport suite; prior task owns provisioning tests; existing schema suite remains sampled | pending |
