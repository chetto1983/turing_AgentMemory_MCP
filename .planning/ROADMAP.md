...
- [x] 05-04-PLAN.md — Provision and reconcile tenant databases with ready-last lifecycle semantics (ARC-07)

**Wave 3 — Runtime routing**

- [x] 05-05-PLAN.md — Build immutable tenant store views with single-flight provisioning and bounded caching (ARC-07)

**Wave 4 — Foreground integration**

- [x] 05-06-PLAN.md — Route foreground server tools through exact tenant-bound stores and layered health checks (ARC-07)

**Wave 5 — Background integration** *(blocked on Wave 4's document-manager resolver contract)*

- [x] 05-07-PLAN.md — Route uploads and document workers without identity transformation or cross-tenant reuse (ARC-07, TEST-05)
...
- [x] 05-09-PLAN.md — Bind a keyed tenant identity into every routed store and fail closed on a foreign identifier (ARC-07)

**Wave 8 — Gap closure: binding reachability**

- [x] 05-10-PLAN.md — Guard every public store path before any span and add the adversarial binding matrix (ARC-07, TEST-05)

**Wave 9 — Gap closure: pseudonymous telemetry** *(closes verification gap 2 — raw identity in shared spans/audits)*

- [x] 05-11-PLAN.md — Sanitize span and audit events centrally to opaque tenant correlation (ARC-07)
...
- [x] 06-02-PLAN.md — Phase-7 fail-closed guard: D-09 schema validator + assert_gate_go (TDD)
...
- [x] 06-04-PLAN.md — Compute + commit gate artifact (gate-result.json + GATE.md) + committed-artifact verification tests
...
- [x] 07-04-PLAN.md — Remove the sys.modules[turingdb] stub from all 31 test files (strictly after src-side imports gone)
- [x] 07-05-PLAN.md — DEP-01/DEP-02 compat-smoke tests + src-wide no-`import turingdb` grep-gate
- [x] 07-06-PLAN.md — Docs/skill/CHANGELOG sweep + Lab/frontend UI ArcadeDB labels
- [x] 07-07-PLAN.md — Rewrite CLAUDE.md + .claude/CLAUDE.md invariants for the ArcadeDB reality
...
- [ ] 07.1-01-PLAN.md — Replace the Phase 07.1 roadmap stub and register the six new requirement IDs while preserving no-orphans coverage.
- [ ] 07.1-02-PLAN.md — Extract the document ingest write path into a new mixin to create 600-LOC headroom with zero behavior change.
...
- [ ] 07.1-13-PLAN.md — Add the flag-gated, hub-damped multi-hop document graph channel to fusion.
- [ ] 07.1-14-PLAN.md — Run the entity-resolution mini-gate and decide SAME_AS from measured residual fragmentation.
- [ ] 07.1-15-PLAN.md — Compose, verify, and measure a 30–50-question MuSiQue-style three-arm evaluation.
- [ ] 07.1-16-PLAN.md — Apply the pre-committed gate, record the verdict, and execute its disposition.
- [ ] 07.1-17-PLAN.md — Wire hub damping and name-only entity seeding into the existing memory graph channel.
...
| 7.1. Document Graph RAG and GLiNER GPU (INSERTED) | 0/17 | Not started | - |
